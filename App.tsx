...
const App: React.FC = () => {
  ...
  const toggleCollapse = () => {
    if (isCollapsed) {
      setInspectorHeight(preCollapseHeightRef.current);
    ...
    <div className={`flex h-screen flex-col overflow-hidden select-none bg-slate-950 text-slate-200 ${isResizing ? 'cursor-ns-resize' : ''}`}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-slate-950 relative">
          ...
          {/* Resizable Inspector Tabs */}
          <div 
            className={`border-t border-slate-800 bg-slate-950 flex flex-col shadow-2xl transition-all ${isResizing ? 'transition-none' : ''}`}
            style={{ height: `${inspectorHeight}px` }}
          >
            {/* Resizer Handle */}
            <div 
              onMouseDown={startResizing}
              className="h-1 cursor-ns-resize hover:bg-cyan-500/50 transition-colors flex items-center justify-center group"
            >
              <div className="w-12 h-0.5 bg-slate-800 group-hover:bg-cyan-500 rounded-full transition-colors" />
            </div>
            ...
            <div className="h-10 bg-slate-900/50 border-b border-slate-800 flex items-center px-4 justify-between select-none">
              ...
              <div className="flex items-center gap-4 text-[10px] font-mono text-slate-500">
                {!isCollapsed && activeTab === 'HEX' && (
                  <div className="flex items-center gap-2">
                    <SearchCode className="w-3.5 h-3.5" />
                    <input type="text" placeholder="Find pattern..." value={hexSearchTerm} onChange={e => setHexSearchTerm(e.target.value)} className="bg-transparent border-none outline-none text-slate-300 w-48" />
                  </div>
                )}
                {!isCollapsed && selectedPacket && <span>LEN: <span className="text-cyan-500 font-bold">{selectedPacket.length}</span> BYTES</span>}
